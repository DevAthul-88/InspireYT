...
export function PricingCards({ userId, subscriptionPlan }: PricingCardsProps) {
  ...
  const PricingCard = ({ offer }: { offer: SubscriptionPlan }) => {
    return (
      <div
        className={cn(
          "relative flex flex-col overflow-hidden rounded-3xl bg-white dark:bg-dark-200 border shadow-sm",
          offer.title.toLocaleLowerCase() === "pro"
            ? "-m-0.5 border-2 border-[#b1e346]"
            : "",
          // Increase card size
          "w-full lg:w-[400px] lg:h-[600px]"
        )}
        key={offer.title}
      >
        <div className="min-h-[150px] items-start space-y-4 bg-muted/50 p-8">
          ...
          <ul className="space-y-3 text-left text-base font-medium leading-normal">
            {offer.benefits.map((feature) => (
              <li className="flex items-start gap-x-3" key={feature}>
                <Icons.check className="size-5 shrink-0 text-[#b1e346]" />
                <p>{feature}</p>
              </li>
            ))}
